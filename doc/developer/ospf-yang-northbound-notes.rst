...
.. warning::
   ...
   in this branch.  The explicit-key, IPsec SA and auth-trailer leaves remain
   on the RFC schema because ``pyang`` and ``libyang`` currently reject the
   deviation paths through their nested choice and case nodes.  Until that is
   resolved, writes to those leaves may be accepted by mgmtd without changing
   daemon state.  The user documentation lists only the supported key-chain
   surface.
...
|                               | as active runtime mirror;   | with ``ospf6_bfd_reg_``     | enable materialises daemon  |
|                               | ``ospf_interface_enable_``  | ``dereg_all_nbr``           | BFD state from stored YANG  |
|                               | ``bfd`` + ``..._bfd_apply`` |                             | parameter leaves; disable   |
|                               |                             |                             | or destroy tears every      |
|                               |                             |                             | session down.               |
|                               |                             |                             | ``[quick]`` (v2) and        |
|                               |                             |                             | ``[profile X]`` (v3) have   |
...
|                               | destroy restores            | destroy restores            | parameter but does not      |
|                               | ``BFD_DEF_DETECT_MULT``     | ``BFD_DEF_DETECT_MULT``     | activate BFD; when enabled, |
...
* ``ospf/fast-reroute/lfa`` and ``interface/fast-reroute/lfa/*``
  ...
  mutation path.
* ``ospf/address-family`` -- fixed per daemon (``ospfv2`` for ``ospfd``,
  ``ospfv3`` for ``ospf6d``); no operator-writable knob.
* ``interface/authentication/(auth-key-explicit|ospfv2-auth-trailer-rfc)``
  and the OSPFv3 ``ospfv3-sa`` / ``ospfv3-key`` / ``ospfv3-sa-id``
  branches -- the legacy CLI supports them but ``pyang`` / ``libyang``
  refuse the deviation paths that would let us declare FRR defaults for
  these nested ``choice`` / ``case`` leaves, so mgmtd silently accepts
  writes against them with no callback fire.  Implementation is held
  until the deviation path resolves; the matching legacy CLI commands
  stay on the direct mutation path.
...
9129's interface-name leafref, so the OSPF model is satisfied without deviating
from RFC 9129. In netns-backed VRF mode, OSPF and zebra both emit
